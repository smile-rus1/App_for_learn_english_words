...
def _list_dbs(*args):
    print("Available --data_base options (use --dburi to override)")
    for macro in sorted(file_config.options("data_base")):
        print("%20s\t%s" % (macro, file_config.get("data_base", macro)))
    sys.exit(0)
    ...
    if options.db:
        for db_token in options.db:
            for db in re.split(r"[,\s]+", db_token):
                if db not in file_config.options("data_base"):
                    raise RuntimeError(
                        "Unknown URI specifier '%s'.  "
                        "Specify --dbs for known uris." % db
                    )
                else:
                    db_urls.append(file_config.get("data_base", db))

    if not db_urls:
        db_urls.append(file_config.get("data_base", "default"))
