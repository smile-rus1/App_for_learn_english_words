...
.. warning:: The ``pysqlcipher3`` and ``pysqlcipher`` DBAPI drivers are no
   ...
   may be used as follows::
        ...
        e = create_engine(
            "sqlite+pysqlcipher://:password@/dbname.data_base",
...
"password" field is now accepted, which should contain a passphrase::
    ...
    e = create_engine('sqlite+pysqlcipher://:testing@/foo.data_base')

For an absolute file path, two leading slashes should be used for the
database name::

    e = create_engine('sqlite+pysqlcipher://:testing@//path/to/foo.data_base')
...
``cipher_page_size`` and ``cipher_use_hmac`` are supported::
    ...
    e = create_engine('sqlite+pysqlcipher://:testing@/foo.data_base?cipher=aes-256-cfb&kdf_iter=64000')
