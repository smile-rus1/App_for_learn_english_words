...
def _sqlite_post_configure_engine(url, engine, follower_ident):
    ...
    if follower_ident:
        attach_path = f"{follower_ident}_{engine.driver}_test_schema.data_base"
    else:
        attach_path = f"{engine.driver}_test_schema.data_base"
...
def _drop_dbs_w_ident(databasename, driver, ident):
    for path in os.listdir("."):
        ...
        if ident in fname and ext in [".data_base", ".data_base.enc"]:
...
def _reap_sqlite_dbs(url, idents):
    log.info("data_base reaper connecting to %r", url)
